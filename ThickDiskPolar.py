#Este � versao final, desconsiderar vers�o 1
from numpy import sqrt,arctan,cos,sin,linspace,zeros,arange,searchsorted,array,pi,ones,copy,tan,empty,arctan2,meshgrid,tile
...
def IntegrationInterval(i, state):
    ...
    if state==0: #Entrando no disco
        for s in x:
            if Disk(1/sol.sol(s)[0], sol.sol(s)[2])==1:
                return s
    else: #Saindo do disco
        sp = x[0]
        for s in x:
            if Disk(1/sol.sol(s)[0], sol.sol(s)[2])==0:
                return sp
            sp = s
...
def Integrand(s):
    ...
    return Disk(1/sol.sol(s)[0], sol.sol(s)[2])*sqrt((sol.sol(s)[1]/(sol.sol(s)[0]**2))**2 + (sol.sol(s)[3]/sol.sol(s)[0])**2)
...
for b in ListaParametros:
    ...
    if Disk(1/sol.sol(sol.t[int(LimitPos[0])])[0], sol.sol(sol.t[int(LimitPos[0])])[2])==1:
        print("erro1", k, b)
        k += 1
        continue
    else:
    ...
    for i in arange(LimitPos[0], LimitPos[1]):
        if Disk(1/sol.sol(sol.t[i])[0], sol.sol(sol.t[i])[2]) != state:
            if state==0:
    ...
    if len(si)!=len(sf):
        if len(sol.t_events[0])!=0:
            if Disk(1/sol.sol(sol.t_events[0][0])[0], sol.sol(sol.t_events[0][0])[2])==1:
                ...
                for s in x:
                    if Disk(1/sol.sol(s)[0], sol.sol(s)[2])==0:
                        ...
                        break
                    sp = s
        elif Disk(1/sol.sol(sol.t_events[1][1])[0], sol.sol(sol.t_events[1][1])[2])==1:
...
for i in range(Nd):
    for j in range(Nd):
        x = (20/Nd)*i - 10
        y = (20/Nd)*j - 10
        AccretionDisk[j][i] = Disk(sqrt(x**2 + y**2), arctan2(y,x))
fig, ax = plt.subplots()
